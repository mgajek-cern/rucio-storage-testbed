...
delegate() {
  ...
  local fts_container
  fts_container=$(docker ps --filter "name=fts-multiarch-build-fts" --format "{{.Names}}" | head -1)
...
for i in $(seq 1 20); do
  code=$(docker exec fts-multiarch-build-fts-1 \
    curl -sk \
...
http_check "PROPFIND /" \
  "$(docker exec fts-multiarch-build-fts-1 \
    curl -sk \
      --capath /etc/grid-security/certificates/ \
      --cert /etc/grid-security/hostcert.pem \
      --key  /etc/grid-security/hostkey.pem \
...
docker exec fts-multiarch-build-xrd1-1 bash -c   'echo "fts-test" > /rucio/fts-test-file && chown xrootd:xrootd /rucio/fts-test-file'
echo "  xrd1 seeded"

seed_code=$(docker exec fts-multiarch-build-fts-1   curl -sk     --capath /etc/grid-security/certificates/     --cert /etc/grid-security/hostcert.pem     --key  /etc/grid-security/hostkey.pem     -X PUT https://webdav1/fts-test-file-from-xrd1     --data-binary "fts-test"     -o /dev/null -w '%{http_code}')
...
docker exec fts-multiarch-build-xrd2-1 ls -la /rucio/fts-test-file-from-webdav
